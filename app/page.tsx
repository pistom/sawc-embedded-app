...
export default function Home() {
  ...
  const [isOn, setIsOn] = useState<{[n: number]: boolean}>({});

  const handleClick = (plant: number) => {
    fetch(`http://${process.env.controllerIP}:3001/output/${plant}/${isOn[plant] ? 'off' : 'on'}`)
      .then(() => setIsOn({...isOn, [plant]: !isOn[plant]}))
      ...
      {[1,2,3,4].map((plant) => ( 
        <div key={`plant_${plant}`}>
          Plant {plant} <input
            ...
            role="switch"
            id={`plant_${plant}`}
            checked={isOn[plant] || false}
            onChange={() => handleClick(plant)} />
          <label
            className="inline-block pl-[0.15rem] hover:cursor-pointer"
            htmlFor={`plant_${plant}`}
          >{isOn[plant] ? "On" : "Off"}</label>
        </div>
      ))}
      </div>
    </div>
