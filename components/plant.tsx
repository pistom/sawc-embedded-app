...
export default function Plant({ output, socket, device }: { output: number, socket: any, device: string }) {
  ...
  const handleMessageSubmit = (output: number, duration: number) => {
    if (!isOn) {
      setIsOn(true);
      socket.emit("message", { action: 'startWater', device, output, duration });
    } else {
      setIsOn(false);
      socket.emit("message", { action: 'stopWater', device, output });
    }
  };

  return (
    <div key={`plant_${output}`}>
      Plant {output} <input
